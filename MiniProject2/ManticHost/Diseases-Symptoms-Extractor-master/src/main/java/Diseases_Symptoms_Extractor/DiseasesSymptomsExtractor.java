	...
	public static void diseaseSymptomsExtractor() throws Exception {
		...
		for (CoreMap sentence : doc.get(CoreAnnotations.SentencesAnnotation.class)) {
			...
			for (RelationTriple triple : triples) {
				...
				boolean isRelationFound;
                            isRelationFound = Arrays.stream(relationDict).parallel().anyMatch(triple.relationLemmaGloss().toLowerCase()::contains);
				//Checking if Disease as subject matching with any Disease in our Disease dictionary
				boolean isDiseaseSubFound;
                            isDiseaseSubFound = Arrays.stream(diseasesDict).parallel().anyMatch((<any> name) -> name.contains());
				//Checking if Disease as object matching with any Disease in our Disease dictionary
				boolean isDiseaseObjFound;
                            isDiseaseObjFound = Arrays.stream(diseasesDict).parallel().anyMatch(triple.objectLemmaGloss().toLowerCase()::contains);
